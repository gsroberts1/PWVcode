function [ v ] =lin3d( IMAGE2, x,y,z)

if(round([x y x])==[x y z])
   if( ( x<1) || (y<1) || (z<1) )
    v=0;
   elseif (  ( x>=size(IMAGE2,1)) || ( y>=size(IMAGE2,2)) || ( z>=size(IMAGE2,3)) )
    v=0;
   else
    v = IMAGE2(x,y,z);
   end
...
v =(mdelx*(IMAGE2(sx  ,sy  ,sz  )*mdely + ... 
           IMAGE2(sx  ,sy+1,sz  )*dely) + ... 
    delx* (IMAGE2(sx+1,sy  ,sz  )*mdely + ...
           IMAGE2(sx+1,sy+1,sz  )*dely) )*mdelz + ...
   (mdelx*(IMAGE2(sx  ,sy  ,sz+1)*mdely + ... 
           IMAGE2(sx  ,sy+1,sz+1)*dely) + ... 
    delx* (IMAGE2(sx+1,sy  ,sz+1)*mdely + ...
           IMAGE2(sx+1,sy+1,sz+1)*dely) )* delz;
end
