...
hold on
%TEMP axis equal tight off vis3d;
...
for t=1:tframes
    ...
    if size(xB,3) == 1
        %slice_CD = interp2(slice_CD,pol);
        vx = interp2(vx_slice(:,:,:,t),pol);
        vy = interp2(vy_slice(:,:,:,t),pol);
        vz = interp2(vz_slice(:,:,:,t),pol);
    else
        %slice_CD = interp3(slice_CD,pol);
        vx = interp3(vx_slice(:,:,:,t),pol);
        vy = interp3(vy_slice(:,:,:,t),pol);
        vz = interp3(vz_slice(:,:,:,t),pol); 
    end
    
%%%Test Plot Normal
Norm = RT*[0; 0; 1];
slice_VEL(:,:,:,t) = Norm(1)*vy_slice(:,:,:,t) + Norm(2)*vx_slice(:,:,:,t) + Norm(3)*vz_slice(:,:,:,t);
...
for t = 1:tframes
    mean_slice_VEL = mean(slice_VEL,3);
    masked_vel_t = slice_VEL(:,:,:,t).*double(slice_MASK);
    mask_vel = mean(masked_vel_t,3);
    mean_vel_t(t)= mean(squeeze(sum(sum(mask_vel,1),2))); 
    mean_vel_t(t) = mean(mean(mask_vel,1));
    time(t) = t*tres;
end

masked_vel_size = size(masked_vel_t);
masked_vel_size = size(slice_VEL);

for t = 1:tframes
    mean_tot_vel(t) = mean(mean(mean(slice_VEL(:,:,:,t)))); 
end
mean_vel_size = mean_tot_vel;
% mean(vel_profile);


%%%Update figure: Pseudo-doppler data
    
%axes(handles.pseudo_doppler)
% edges = 1:1:max(max(abs(profile))); 
%     for t = 1:tframes
%       masked_vel_t = slice_VEL(:,:,:,t).*double(slice_MASK);
%       vel_size = size(masked_vel_t);
%       vector_size = vel_size(1)*vel_size(2)*vel_size(3);
%       reshape_vel = abs(reshape(masked_vel_t,vector_size,1));
%       vel_hist(:,t) = histc(reshape_vel,edges);
%     
%     end
%     vel_hist;
%     plot(vel_hist)
...
%area = pi * (7.9375/2)^2;
%flow_mean = mean_vel_t;
mean_velocity = mean_vel_t;
flow_mean = mean_vel_t * delX * delY / 1000;
%flow_mean = flow_t *voxels*delX*delY / 1000;
%flow_mean = mean_vel_t * area / 1000;

%plot(flow_mean);
plot(time,mean_velocity);
xlabel('time [ms]');
ylabel('velocity [ml/s]');
end

%**************************************************************************
%%Values displayed on GUI
%**************************************************************************
max_vel = 0;
total_flow = 0;

max_vel = max(max(abs(profile))); 
...
if(CINE_flag == 1)
   ...
   csvwrite(flow_name,mean_velocity);
