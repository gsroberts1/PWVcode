...
%%%RUN Visualization GUI (don't use anymore now we have ensight)
%if visual_flag == 1
%    uiwait( visual_gui());
%end
...
%             end
        ...
        if grad_export_flag ==1
            ...
            imageMxFlow(:,:,:,2) = imagePress* signVx; 
            
            imagePress(plist) = GRADy(:,phase);
            imageMxFlow(:,:,:,1) = imagePress* signVy; 
...
%             dl = 2;
%             outStr = ['Dicom --> EnSight --> Dialating Pressure']
%             disp(outStr);
%             drawnow;
% 
             imagePress = zeros(size(sCD));
...
%             
%             %%%Dialate the Pressure Matrix
%             kernel = ones(2*dl+1,2*dl+1,2*dl+1);
%             imageMask = sMask;
%             
%             %%3D Convolution
%             imageMask=convn(imageMask,kernel,'same');
%             imagePress=convn(imagePress,kernel,'same');
%             
%             %%Convert to average
%             imagePress = imagePress./(0.00001+imageMask);
%                         
%            imagePress(plist) = 0.007501*PRESSURE(:,phase);
...
%             % Dialate the Pressure Matrix by dl (BAD LOOP)
%             dl = 3;
%             for xx=dl+1:m_xlength-dl-1
%                 outStr = ['Dicom --> EnSight --> Dialating Pressure Mask slice',num2str(xx),' of ',num2str(m_xlength),' ...'];
%                 disp(outStr);
%                 drawnow;
%                 for yy=dl+1:m_ylength-dl-1
%                 for zz=dl+1:m_zlength-dl-1
%                     if(sMASK(xx,yy,zz)==0)
%                     xrange =  xx - dl : xx + dl; 
%                     yrange =  yy - dl : yy + dl;
%                     zrange =  zz - dl : zz + dl;
%                     
%                     t_mask = ( sMASK(xrange,yrange,zrange) > 0);
%                     t_pressure = imagePress(xrange,yrange,zrange);
%                     
%                     if(sum(t_mask(:)) > 0)
%                         imagePress(xx,yy,zz) = sum(t_pressure(:))/(0.000001+sum(t_mask(:)));
%                     end
%                     
%                     end
%                 end
